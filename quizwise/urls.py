"""
URL configuration for quizwise project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""

from django.contrib import admin
from django.urls import path
from rest_framework.authtoken.views import ObtainAuthToken

from quiz_app.views import (
    SignUpView,
    QuizCreateListView,
    QuizRetrieveView,
    QuizDestroyView,
    QuestionCreateView,
    ChoiceCreateView,
    QuizAttemptCreateView,
    AnswerCreateView,
    LanguageCreateView,
    QuizResultCreateView
)

urlpatterns = [
    path('admin/', admin.site.urls),
    path('register/', SignUpView.as_view()),
    path('token/', ObtainAuthToken.as_view()),
    path('language/',LanguageCreateView.as_view()),
    path('quiz/', QuizCreateListView.as_view()),
    path('quiz/<int:pk>/', QuizRetrieveView.as_view()),
    path('quiz/<int:pk>/remove/', QuizDestroyView.as_view()),
    path('quiz/<int:pk>/add-question/', QuestionCreateView.as_view()),
    path('question/<int:pk>/add-choice/', ChoiceCreateView.as_view()),
    path('quiz/<int:pk>/start/', QuizAttemptCreateView.as_view()),
    path('attempt/<int:pk>/answer/', AnswerCreateView.as_view()),
    path('result/<int:pk>/',QuizResultCreateView.as_view()),
]